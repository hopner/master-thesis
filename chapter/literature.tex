...
In this project, the \texttt{krippendorff} library for Python\ \cite{castro-2017-fast-krippendorff} was used to calculate \(\alpha \) for each category, which differs slightly from the above formula. Instead of using the just the raw-count coincidence matrix, it constructs a coincidence matrix that is weighted and normalized per unit.
...
This raises the question if we have failed in educating lay audiences about the capabilities and limitations of \gls{llms}, and how to best aaddress the misconceptions that may arise from the widespread use of these systems.
...
The \gls{zpd}, introduced by Vygotsky, describes the space between what a learner can do independently and what they can do with guidance (\autoref{zpd})\ \cite{vygotskij_mind_1981}. It argues that what is positioned in \gls{zpd} for learners today, will be within actual development tomorrow, and therefore learning is most effective when learning activities are within this zone. As a consequence, the \gls{zpd} shifts as competence increases, and the learner is able to perform more complex tasks independently. Similarily, we have the concept of ``flow'', introduced by Csikszentmihalyi, which describes a state of complete immersion and focus in an activity (\autoref{flow})\ \cite{csikszentmihalyi_beyond_1977}. Flow is acomplished when the challenge of an activity is balanced with the skill level of the individual.
\begin{figure}[H]
\centering

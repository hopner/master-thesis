...
The components selected for mediation are mainly chosen to address misconception in the dimension of \hyperref[dim2]{Mechanistic Understanding}, in hope that by giving users a better understanding of how \gls{llms} work, they will be better equipped to use them in a responsible manner.
...
This category is perhaps the most true to real \gls{llms}, as the features in this category actually modifies the training data of the model. By understanding training data, you get an understanding on how \gls{llms} are somehow fixed and does not gain new knowledge with interaction from the user. Thus, it will prevent the \hyperref[realtime]{REALTIME} misconception. Also, by understanding that the tokens used for training have no tracing to were in the training data they come from, it will prevent the \hyperref[sourceattr]{SOURCEATTR} misconception.

The first feature the users can add is the ``Read Book''-feature, which simply adds a book from Project Gutenberg\ \cite{gutenberg} to the training data of the model. Before adding this feature, the model works by randomly selecting a a word from the entire english language as the next token. After adding the feature, the model will only select words from the book that was added to the training data. The learning target of this feature is to introduce the idea that models are trained on previously written human text, in hope that it will learn the patterns and structures of human language. 
...
An important aspect of \gls{llms} is that they are non-deterministic, meaning that the same prompt can produce different outputs, which is done to make more varied output. The addition of this variability comes with the cost of unpredictability and also the risk of generated output not being true to the training data. Understanding the nature of how this randomness is introduced into the model is important for responsible use of \gls{llms}. We added this feature to prevent the \hyperref[hallblind]{HALLBLIND} misconception.
...
After the assessment interview, participants were given access to the mediation tool and were asked to use it during the upcoming week. They were not given any time requirement for how much they should use the tool, but were encouraged to explore it at their own pace and in their own time. The mediation tool was hosted on a virtual machine in \gls{uib}'s cloud infrastructure service, NREC\ \cite{nrec}. Participants were given a link to the tool, as well as a brief introduction to how to access and use it. They were also given contact information of the researcher in case they had any questions or issues with the tool.
...
After coding of the assessment interviews, the data was analyzed through \gls{irr} analysis to determine the level of agreement between the coders. The analysis was done in a Jupyter Notebook using \gls{py} 3.12. The \gls{irr} was calculated using percentage of agreement, Cohen's Kappa, calculated with the \texttt{cohen\_kappa\_score} function from the \texttt{sklearn.metrics} library\ \cite{scikit-learn}, and Krippendorff's Alpha, calculated with the \texttt{krippendorff.alpha} function from the \texttt{krippendorff} library\ \cite{castro-2017-fast-krippendorff}. A \gls{irr} coefficient was calculated for each misconception category, treating the presence or absence of the misconception in each transcript as a binary variable. 

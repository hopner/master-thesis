...
The mediation tool does not actually contain a real transformer-based \gls{llm} under the hood, but rather a simple representation of one. Each branch of the skill tree corresponds to a different feature or component of \gls{llms}. Most of the topics contain multiple chapters building on each other, to keep users in the zone of proximal flow\ \cite{basawapatna_zones_2013}. The features are grouped into 4 different categories: training data, context, non-determinism and tokenization. Each of the categories are aimed at giving intuition on how the components work rather than being technically accurate, and the different features are designed to demonstrate the effect of that component on the output of the model. The selection tree, with corresponding topic, can be seen in\ \autoref{fig:selection-tree}. 
